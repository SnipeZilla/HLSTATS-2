...
public hlx_amx_browse(id, level, cid)
{
    ...
    copy(name, charsmax(name), name[1]);
    new raw_user_id = str_to_num(name);
    ...
    for (new i = 0; i < player_count; i++) {
        new p = Players[i];
        if (get_user_userid(p) == raw_user_id) { userid = p; break; }
    }
    ...
    static url[1024];
    read_argv(2, url, charsmax(url));
    remove_quotes(url);

    replace_all(url, charsmax(url), "\n", "");
    replace_all(url, charsmax(url), "\r", "");

    static motd[1400];
    formatex(motd, charsmax(motd),"<html><head><meta http-equiv='refresh' content='0;url=%s'></head><body>Loading...</body></html>", url);

    show_motd(userid, motd, "HLstatsX CE");
    return PLUGIN_HANDLED;
}
